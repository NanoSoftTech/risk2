...
<?php } ?>
                ...
                <div class="col-sm-12">
                    <div class="form-group"> 
                    <?php 
                    if($record['risk_status'] < 2 && $record['user_id'] == $userid ){ 
                    echo    "<a href='?page=".sha1('edit_risk')."&getid=$record[risk_id]'>
                        <input type='submit' name='submit' id='submit' class='form-control btn-warning' value='แก้ไข'>
                        </a>";} 
                    if($record['risk_status'] == 1 && $user_sta == '2' ){ 
                    echo    "<a href='?page=".sha1('edit_man_risk')."&getid=$record[risk_id]' onclick='return checkUpdate()'>
                        <input type='submit' name='submit' id='submit' class='form-control btn-info' value='รับเรื่อง'>
                        </a>";}
                     if($record['risk_status'] == 2 && $user_sta == '2'  ){ 
                    echo    "<a href='?page=".sha1('edit_man_risk')."&getid=$record[risk_id]' onclick='return checkUpdate()' >
                        <input type='submit' name='submit' id='submit' class='form-control btn-danger' value='เสร็จสิ้นการพิจารณา'>
                        </a>";} 
                     ?>
                    </div>
                </div>
